...
av_av<-cbind(ind_matrix_af[,-c(7)],social_matrix_af, ac_matrix_af,health)
```
```{r}
#normalize data
area<-subset(data,Parameter.=="Area of the country")    
area_melt<-melt(area,id.vars = c("Parameter.","Unit.","Source.","Year."))
area_melt$value<-as.numeric(area_melt$value)
area_c<-as.numeric(area_melt$value)
area_af<-head(area_c,-5)
ind_matrix_af<-as.data.frame(ind_matrix_af)
ind_matrix_af$tot_re_w_c<-ind_matrix_af$tot_re_w_c/area_af*10^6
ind_matrix_af$tot_re_w_c<-as.numeric(ind_matrix_af$tot_re_w_c)
...
ind_matrix_pca<- prcomp(ind_matrix_pca[1:5,],center=TRUE, scale = TRUE)#matrix was normalized (scale=true) before perform pca analysis
...
#+ geom_hline(yintercept = avg_lt_ppt_c)
warning= FALSE
hist(ind_matrix_af$tot_re_w_c, breaks = 20, xlab="log10(total renewable water resources), log10(cubic meter per year per hectare area of the country)", main = "Histogram: Log10 of Total Renewable Water Resources of SSA Countries")
```
...
The log10 values of total renewable water resources was used in cluster analysis. The data frame was scaled for the cluster analysis.  
...
#select and subset data of two matrices and diff lengths
judge<-ifelse(row.names(ind_matrix_af) %in% row.names(ca_data_se),row(ind_matrix_af),0)
judge<-subset(judge, judge>0)
ind_matrix_af_pl<-ind_matrix_af[judge,]
library(RColorBrewer)
ggplot(ind_matrix_af_pl, aes(x=hdi_c, y=var_seas_c, color=as.character(ca_data_se$fit_se.cluster)))+
  geom_point()+ geom_smooth(method="lm", color="black") +
  labs(x="Human Development Index", y= "Seasonal Variability")+
  theme(legend.position = "none")+ geom_text_repel(aes(hdi_c, var_seas_c, label = row.names(ind_matrix_af_pl)))
warning= FALSE
?ggplot
```
...
```{r}
#pairs(~lt_ppt_c+tot_re_w_c+dep_ra_c+var_interan_c+var_seas_c+gdp_c+gdp_pp_c+edu_ind_c+life_exp_c, data= av_av)
...
pairs(~lt_ppt_c+tot_re_w_c+dep_ra_c+var_interan_c+var_seas_c+gdp_c+gdp_pp_c+edu_ind_c+life_exp_c+tot_pop_ac_dw+tot_pop_impr_sani+ener_pp+defi_food+diarrhea+death_airpollu+U5underw, data= av_av,
