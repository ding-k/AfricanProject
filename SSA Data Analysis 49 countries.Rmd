...
#+ geom_hline(yintercept = avg_lt_ppt_c)
warning= FALSE
hist(hist(ind_matrix_af$tot_re_w_c))
```
...
# K-Means Cluster Analysis
fit <- kmeans(ind_wo_N_SA_ca_se, 5) # 5 cluster solution
# get cluster means 
aggregate(ind_wo_N_SA_ca_se,by=list(fit$cluster),FUN=mean)
# append cluster assignment
ca_data <- data.frame(ind_wo_N_SA_ca_se, fit$cluster)
